<template>
  ...
  <div class="Atest">
    <div class="test_top">
      <div class="test_style">【不定项选择题】（{{ tdata.score }}分）
      ...
      <div class="test_title">{{ tdata.testNO }}、
        <span v-if="tdata.frontTitle" v-html="tdata.frontTitle+'<br>'+tdata.title"></span>
        <span v-else v-html="tdata.title"></span>
    ...
    <div class="test_bot">
      <dl>
        <dd v-for="item in tdata.selectedItems" :key="item.ItemName" @click="answerClick(item.ItemName)">
          <span v-if="userReply.includes(item.ItemName)" class="radioA RSelect">{{ item.ItemName }}</span>
    ...
    <!-- <script src="../js/txplayer.js"></script> -->
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    init () {
      ...
      // if (this.tdata.hasVideo && this.currentIndex === this.tdata.testNO) {
      //   var player = new Txplayer(
      //     {
      //       containerId: domID,
      //       vid: vid,
      //       width: '100%',
      //       height: screen.availWidth * 9 / 16,
      //       removePluginsList: ['UiBrowserFullScreen', 'UiWindowFullScreen']
      //     }
      //   )
      // }
    },
    answerClick (userReply) {
      ...
      const select = userReply
      this.showsubmitBtn = true
      if (this.userReply.includes(select)) {
        // 如果选项中包含所点内容，则删除所选内容
        this.userReply = this.userReply.split(select).join('')
      } else {
        this.userReply += select
        this.userReply = this.sortAnswer(this.userReply)
      }
    },
    // 给答案排序
    sortAnswer: function (answer) {
